...
const MobileOrderCard = ({ order }) => {
  ...
  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      ...
      default:
        return 'text-[#8d4b00] bg-[#fdeade] border border-[#e9d7cb]';
    }
  };
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-[#e9d7cb] hover:border-[#8d4b00]/30 rounded-2xl p-5 mb-4 shadow-sm hover:shadow-md transition-all duration-300 group"
    >
      <Link to={`/orders/${order.id}`}>
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-xl bg-[#fdeade] text-[#8d4b00] flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform duration-300">
              <FiPackage className="text-xl" />
            </div>
            <div>
              <h3 className="font-bold text-gray-800 text-base group-hover:text-[#8d4b00] transition-colors">
                Order #{order.id}
              </h3>
          ...
          <FiChevronRight className="text-gray-400 group-hover:text-[#8d4b00] group-hover:translate-x-1 transition-all text-xl" />
        </div>

        <div className="space-y-2.5 mb-4">
          {/* Vendor Count */}
          {order.vendorItems && order.vendorItems.length > 0 && (
            <div className="flex items-center gap-2 px-2.5 py-1.5 bg-[#fff8f5] border border-[#e9d7cb]/40 rounded-xl mb-3">
              <FiShoppingBag className="text-[#8d4b00] text-xs" />
          ...
          <div className="flex items-center justify-between text-sm pt-0.5">
            ...
            <span className="text-base font-bold text-[#8d4b00]">
        ...
        {order.shippingAddress && (
          <div className="flex items-start gap-2.5 px-3.5 py-2.5 bg-[#fff8f5] rounded-xl border border-[#e9d7cb]/40 mb-4 text-xs">
            <FiMapPin className="text-[#8d4b00] text-sm mt-0.5 flex-shrink-0" />
            <div className="text-[#554336] leading-relaxed">
              <p className="font-bold text-gray-700 mb-0.5">Ship To</p>
              <p className="font-semibold text-[#8d4b00]">{order.shippingAddress.fullName}</p>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <span className="text-xs font-bold text-[#8d4b00] group-hover:underline flex items-center gap-0.5">
            View Details
